...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  style="position: fixed; bottom: 20px; right: 20px; width: 400px; height: 600px; border: none; border-radius: 24px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); z-index: 9999;"
...
></iframe>`;
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
        <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
          <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-6 flex items-center gap-3">
            <Icons.Zap className="text-rose-600" /> Widget Installation
          </h3>
          <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mb-8 leading-relaxed">
            Copy and paste the following code snippet into the <code className="bg-slate-100 px-1 rounded text-rose-600">&lt;body&gt;</code> tag of your website to install the Express Print AI Assistant.
          </p>
          
          <div className="relative group">
            <div className="bg-slate-900 rounded-2xl p-6 text-slate-300 font-mono text-[10px] overflow-x-auto shadow-2xl border border-slate-800">
              <pre>{embedCode}</pre>
            </div>
            <button 
              onClick={handleCopy}
              className="absolute top-4 right-4 px-4 py-2 bg-white/10 hover:bg-white/20 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all backdrop-blur-md border border-white/10"
            >
          ...
          <div className="mt-8 p-6 bg-rose-50 rounded-2xl border border-rose-100">
            <h4 className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-2 flex items-center gap-2">
              <Icons.TrendingUp className="w-3 h-3" /> Pro Tip
            </h4>
            <p className="text-[11px] text-rose-900 font-medium leading-relaxed">
              You can adjust the <code className="font-bold">width</code> and <code className="font-bold">height</code> in the snippet above to better fit your website's layout. For mobile-only views, we recommend a width of <code className="font-bold">100%</code>.
            </p>
          </div>
        </div>
      </div>
