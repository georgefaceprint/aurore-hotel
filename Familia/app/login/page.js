...
export default function LoginPage() {
    ...
    const handleCheckPhone = async (forceReset = false) => {
        ...
        try {
            const res = await fetch('/api/auth/phone', {
                ...
                body: JSON.stringify({ phone: fullPhone, forceOtp: forceReset })
            });
            const data = await res.json();
            setIsNewUser(!data.exists);
            
            if (!data.exists || forceReset) {
                setStep(2); // Go to OTP
            } else {
                setStep(3); // Go to PIN Entry (step 3 for returning users)
            }
        } catch (err) {
    ...
    const handleVerifyOtp = async () => {
        if (otp.length !== 5) { setError('OTP MUST BE 5 DIGITS'); return; }
        setLoading(true); setError('');
        const fullPhone = phoneCode + phoneNumber.replace(/^0+/, '');
    ...
    const handleAccess = async () => {
        ...
        const needsConfirmation = isNewUser || otpVerified;
        if (needsConfirmation && pin !== confirmPin) { setError('CODES DO NOT MATCH'); return; }
        ...
        try {
            const res = await signIn('phone', {
                phone: fullPhone,
                otp: pin, // This is the PIN
                isNew: isNewUser || otpVerified,
                redirect: false
            });
                ...
                <div className="auth-content">
                    {step === 1 ? (
                        <div className="flow-container animate-slide-up">
                            ...
                            <button onClick={() => handleCheckPhone(false)} disabled={loading} className="btn-primary-cinematic">
                        ...
                        <div className="flow-container animate-slide-up">
                            <div className="input-group">
                                <label>VERIFY YOUR IDENTITY (OTP)</label>
                                <p style={{ fontSize: '0.8rem', color: '#94a3b8', textAlign: 'center', marginBottom: '1.5rem' }}>
                                    Enter the code sent via SMS to {phoneCode} {phoneNumber}
                                </p>
                                <input
                                    type="text"
                                    maxLength={5}
                            ...
                            <button onClick={() => setStep(1)} className="btn-link">BACK</button>
                        </div>
                    ) : step === 3 ? (
                        <div className="flow-container animate-slide-up">
                            <div className="input-group">
                                <label>ENTER YOUR 5-DIGIT ACCESS CODE</label>
                                <input
                                    type="password"
                                    maxLength={5}
                                    placeholder="•••••"
                            ...
                            <button onClick={() => handleCheckPhone(true)} className="btn-link">FORGOT PASSCODE?</button>
                            <button onClick={() => setStep(1)} className="btn-link">CHANGE NUMBER</button>
                        </div>
                    ) : (
                        <div className="flow-container animate-slide-up">
                            <div className="input-group">
                                <label>CREATE YOUR NEW ACCESS CODE</label>
                                <input
                                    type="password"
                                    maxLength={5}
                                    placeholder="•••••"
